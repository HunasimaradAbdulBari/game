export class AnimationManager {
    ...
    createEntranceAnimation(target, type = 'fade', delay = 0) {
        if (!target || !this.scene || this.isSceneDestroyed()) return null;
    ...
    createHoverEffect(button, scale = 1.1) {
        if (!button || !this.scene || this.isSceneDestroyed()) return;
    ...
    createParticleExplosion(x, y, color = 0xffffff, count = 15) {
        if (!this.scene || this.isSceneDestroyed()) return [];
    ...
    isSceneDestroyed() {
        ...
        return this.scene.scene && (this.scene.scene.isDestroyed === true || this.scene.sys && this.scene.sys.isDestroyed === true);
    }
        ...
        this.activeParticles.forEach(particle => {
            if (particle && particle.destroy) {
                try {
                    ...
                    console.warn('Error destroying particle:', e);
                }
            }
        });
