// src/app/market/page.js - Lab Equipment Store (replaces market.js)
'use client';
...
export default function MarketPage() {
  ...
  const handleDragEnd = ({ active, over }) => {
    ...
    // GSAP Animation for new item in basket
    setTimeout(() => {
  ...
  const handleRemoveItem = (idx) => {
    // GSAP Animation for removing item
    if (basketItemsContainerRef.current) {
    ...
    <Layout scene="market" subject={currentSubject}>
      ...
      >
        ...
        >
          ...
          >
            {levelData.available
              .filter(item => !basket.includes(item)) // Only show items NOT in basket
              .map((item, idx) => (
              <div
                key={`${item}-${idx}`}
            ...
            >
              <DroppableWrapper id="basket">
                <div style={{ position: 'relative' }}>
                  ...
                  >
                    {basket.slice(0, 6).map((item, idx) => {
                      const itemDetails = getItemDetails(item);
                      return (
                        <div
                          key={`mini-${item}-${idx}`}
                          className="basket-mini-item"
                          style={{
                            ...
                            alignItems: 'center',
                            justifyContent: 'center',
                            minWidth: 'clamp(12px, 2vw, 16px)',
                            minHeight: 'clamp(12px, 2vw, 16px)',
